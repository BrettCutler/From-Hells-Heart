...
public class PlayerCharacterController : MonoBehaviour
{
  ...
  private float m_lastTimeSwitchedWalls = float.MinValue;
  private float m_lastTimeKickedOffWall = float.MinValue;

  private Collider2D m_formerCollider;

  const int k_maxNumberOfWallCollisions = 10;
  const float k_minTimeBetweenSwitchingWalls = .2f;
  const float k_disableWallHugAfterJumpDuration = .5f;
  ...
  {    
    ...
    if( m_TouchingWall && hugWall &&
        Time.time - m_lastTimeKickedOffWall > k_disableWallHugAfterJumpDuration )
    {
      ...
      {
        //if( m_formerCollider == null )
        //{
        //  Debug.Log( "switch walls! from null to " + m_WallsCollided[targetIndex].collider.gameObject.name );
        //}
        //else
        //{
        //  Debug.Log( "switch walls! from " + m_formerCollider.gameObject.name + " to " + m_WallsCollided[targetIndex].collider.gameObject.name );
        //}
      ...
      // Get a clean raycast directly through our bottom-center to our target surface.
      // Otherwise, the earlier CircleCast intersects ahead or behind our center point
      // If we can't find a surface, fall back to the collision point from the earlier CircleCast
      ...
      RaycastHit2D rayToCenterBase = Physics2D.Raycast( transform.position, -m_WallsCollided[targetIndex].normal, m_touchingWallRadius, m_WallLayers );

      debugStatement += "\n" + ( "rayToCenterBase; hit anything? " + ( rayToCenterBase.collider != null ) +
        ", ray start pos = " + transform.position +
        ", rayDir = " + -m_WallsCollided[targetIndex].normal +
        ", distance = " + rayToCenterBase.distance +
        ", collider null? " + ( rayToCenterBase.collider == null ) +
        ", direction of ray = " + -transform.up );

      if( rayToCenterBase.collider == m_WallsCollided[targetIndex].collider )
      {
        raycastToWall = rayToCenterBase;
        debugStatement += "\n" + ( "rayCast success! vector = " + ( ( -m_WallsCollided[targetIndex].point + (Vector2)transform.position ) * m_touchingWallRadius ) +
          ", impact point is " + rayToCenterBase.point );
      }
      ...
      //Debug.Log( "footRay hit anything? " + (footRay.collider != null ) + ", footWorldPos = " + footWorldPos + ", footDir = " + -transform.up );

      if( footRay.collider == null )
      {
        reachedEdgeOfSurface = true;
        //Debug.Log( "NO FOOT collider, footWorldPos = " + footWorldPos + ", footDir = " + -transform.up + ", transform.pos = " + transform.position );
      }
      else if( rayToCenterBase.collider == null )
      {
        raycastToWall = footRay;
        //debugStatement += "\n" + ( "switch to FOOT, center ray has missed but foot hasn't." );
        //Debug.Break();
      }
      ///////////////////////////////////////////
      ...
      if( jump )
      {
        outputVelocityMagnitude = m_jumpForce;
        outputVelocity = raycastToWall.normal;
        m_lastTimeKickedOffWall = Time.time;
      }
      else
      {
        ...
        if( raycastToWall.normal != null )
        {
          // DEBUG
          Vector2 oldFacing = transform.up;

          float zAngle = Mathf.Rad2Deg * Mathf.Atan2( raycastToWall.normal.y, raycastToWall.normal.x ) - 90f;
          transform.rotation = Quaternion.Euler( new Vector3( 0f, 0f, zAngle ) );
    ...
    else // add force from input direction
    {
      ...
      outputVelocityMagnitude = Mathf.Min( outputVelocity.magnitude, m_MaxVelocity ) *
        m_VelocityScalarAfterWallKickoff.Evaluate( Time.time - m_lastTimeKickedOffWall );
      ...
      if( moveDir.sqrMagnitude > Mathf.Epsilon )
      {
        float zAngle = Mathf.Rad2Deg * Mathf.Atan2( moveDir.y, moveDir.x ) - 90f;
        transform.rotation = Quaternion.Euler( new Vector3( 0f, 0f, zAngle ) );
      }
    }
